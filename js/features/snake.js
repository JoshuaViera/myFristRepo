...
function initSnakeGame() {
  ...
  // Make canvas focusable to capture keyboard events reliably
  try { snakeCanvas.setAttribute('tabindex','0'); } catch(_){ }
  resetSnakeGame();
...
function resetSnakeGame() {
  snake = [{ x: 10, y: 10 }]; direction = { x: 0, y: 0 }; nextDirection = { x: 0, y: 0 };
  score = 0; snakeLost = false; snakeLoseReason = '';
  const overlay = document.getElementById('snake-lose-overlay'); if (overlay) overlay.style.display = 'none';
  confettiActive = false; confettiParticles = [];
  gameRunning = false; gamePaused = false;
  // restore page scroll on reset
  try { document.body && (document.body.style.overflow = ''); } catch(_){ }
  if (gameLoop) { clearInterval(gameLoop); gameLoop = null; }
  generateFood(); updateSnakeDisplay(); updateSnakeGameStatus('Press SPACE or click Start to begin'); drawSnakeGame();
}

function startSnakeGame() {
  if (!gameRunning && !gamePaused) { direction = { x: 1, y: 0 }; nextDirection = { x: 1, y: 0 }; }
  gameRunning = true; gamePaused = false;
  // prevent page scroll while playing
  try { document.body && (document.body.style.overflow = 'hidden'); } catch(_){ }
  // focus canvas so keys are directed to the game
  try { snakeCanvas && snakeCanvas.focus && snakeCanvas.focus(); } catch(_){ }
  updateSnakeGameStatus('Game Running');
  if (!gameLoop) gameLoop = setInterval(gameUpdate, gameSpeed);
}
...
function pauseSnakeGame() {
  if (gameRunning) {
    ...
    // restore scrolling when paused; hide when resumed
    try { document.body && (document.body.style.overflow = gamePaused ? '' : 'hidden'); } catch(_){ }
    updateSnakeGameStatus(gamePaused ? 'Game Paused' : 'Game Running');
    if (gamePaused) { clearInterval(gameLoop); gameLoop = null; }
    else { gameLoop = setInterval(gameUpdate, gameSpeed); }
  }
}
...
function gameOver() {
  gameRunning = false; gamePaused = false;
  if (gameLoop) { clearInterval(gameLoop); gameLoop = null; }
  if (score > highScore) { highScore = score; localStorage.setItem('snakeHighScore', highScore); document.getElementById('snake-high-score').textContent = highScore; }
  updateSnakeGameStatus(`You lost: ${snakeLoseReason} — Final Score: ${score}`);
  snake = [{ x: 10, y: 10 }]; direction = { x: 0, y: 0 }; nextDirection = { x: 0, y: 0 };
  generateFood(); updateSnakeDisplay(); drawSnakeGame();
  // restore page scroll on game over
  try { document.body && (document.body.style.overflow = ''); } catch(_){ }
  const overlay = document.getElementById('snake-lose-overlay');
  if (overlay) { const title = document.getElementById('snake-lose-title'); const reasonEl = document.getElementById('snake-lose-reason'); if (title) title.textContent = 'You Lost!'; if (reasonEl) reasonEl.textContent = snakeLoseReason; overlay.style.display = 'flex'; overlay.style.alignItems = 'center'; overlay.style.justifyContent = 'center'; overlay.style.pointerEvents = 'auto'; const btn = document.getElementById('snake-restart-btn'); if (btn) btn.focus(); }
  createConfetti();
}
...
function handleSnakeKeyPress(e) {
  // Always prevent page scroll when interacting with snake controls if the canvas exists
  // This stops the browser from scrolling on Arrow keys / Space while playing
  if (document.getElementById('snakeCanvas')) {
    const preventFor = ['ArrowUp','ArrowDown','ArrowLeft','ArrowRight','Space','KeyW','KeyA','KeyS','KeyD'];
    if (preventFor.includes(e.code)) e.preventDefault();
  }

  if (snakeLost && e.code === 'Space') { resetSnakeGame(); startSnakeGame(); return; }
  if (!gameRunning && e.code === 'Space') { startSnakeGame(); return; }
  if (gameRunning && e.code === 'Space') { pauseSnakeGame(); return; }
